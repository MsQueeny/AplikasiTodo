...
const Todo = () => {
  ...
  const getList = async () => {
    axios
      .get(`http://192.168.11.74:3200/todo/${datax.id}`)
      .then(({ data }) => {
  ...
  const getData = async () => {
    try {
      ...
      const check = await axios.get(`http://192.168.11.74:3200/users/${username}`)
            ...
            >
              ...
              <View style={{ top: 28, right: 20, position: 'absolute' }}>
                <Checkbox
                  ...
                  onPress={async () => {
                    if (item.status == 0) {
                      await axios.put(`http://192.168.11.74:3200/todo/status/${item.id}`, { status: 1 })
                    } else {
                      await axios.put(`http://192.168.11.74:3200/todo/status/${item.id}`, { status: 0 })
                    }
                  }}
                />
              </View>
              <TouchableOpacity
                ...
                onPress={async () => {
                  await axios.delete(`http://192.168.11.74:3200/todo/${item.id}`)
                }}>
